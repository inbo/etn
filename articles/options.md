...
To store an option permanently, open the `.Renviron` file with
`usethis::edit_r_environ()`, edit, and save:
...
- `ETN_PROTOCOL` (value: `"opencpu"` or `"localdb"`): etn will
  ...
  will use a local database connection when available. This option
  allows you to override this behaviour by hardcoding the protocol to
  use.

- `ETN_TEST_API` (value: a url) **developers only**: etn uses the
  production version of the OpenCPU API by default. This option allows
  you to use a test version of the OpenCPU API (available upon request).
  Any other value will break the package.

...
testthat::test_that("test input list_values", {
  ...
  expect_error(list_values(df, chr_col, split = 1),
               "split is not a character vector")
  })

testthat::test_that("test output list_values", {
  ...
  expect_equal(list_values(df, "dot_sep_col", split = ","),
               df$dot_sep_col)
})

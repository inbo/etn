...
test_that("get_acoustic_detections() allows selecting on network_project_code", {
  # Errors
  expect_error(get_acoustic_detections(network_project_code = "not_a_project"))
  expect_error(get_acoustic_detections(network_project_code = c("demer", "not_a_project")))

  # Select single value
  single_select <- "demer"
  single_select_df <- get_acoustic_detections(network_project_code = single_select)
  expect_equal(
  ...
  multi_select_df <- get_acoustic_detections(network_project_code = multi_select)
  expect_equal(
...
test_that("get_acoustic_detections() allows to limit to 100 records", {
  ...
  expect_equal(
    nrow(get_acoustic_detections(network_project_code = "demer", limit = TRUE)),
    100
  )
})

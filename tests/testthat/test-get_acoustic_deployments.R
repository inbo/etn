...
test_that("[API] get_acoustic_deployments() allows selecting on deployment_id", {
  ...
  expect_equal(
    single_select_df |> dplyr::distinct(deployment_id) |> dplyr::pull(),
    c(single_select)
  )
  ...
  expect_equal(
    multi_select_df |> dplyr::distinct(deployment_id) |> dplyr::pull() |> sort(),
...
test_that("[API] get_acoustic_deployments() allows selecting on acoustic_project_code", {
  ...
  expect_equal(
    multi_select_df |> dplyr::distinct(acoustic_project_code) |> dplyr::pull() |> sort(),
    c(multi_select)
  )
  expect_gt(nrow(multi_select_df), nrow(single_select_df))
})

test_that("example_dataset() returns error when an invalid dataset name ", {
  expect_error(
...
test_that(
  paste(
    "example_dataset(dataset = '2014_DEMER') returns a valid frictionless",
    "data package"
  ),
  {
    expect_no_error(
...
test_that(
  paste(
    "example_dataset(dataset = '2014_DEMER') returns a data package with",
    "5 valid resources"
  ),
  {
    ...
    expect_equal(
      frictionless::resources(`2014_DEMER`), expected_resource_names
    )

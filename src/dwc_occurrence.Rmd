...
```{r}
raw_data %>%
  select(deployment_raw_column_names) %>%
  group_by_all() %>%
  summarize(detections = n()) %>%
  arrange(detections_receiver) -> deployment
```

And `detections` to `deployment_raw_column_names` (makes it easier to remove them all later):

```{r}
# deployment_raw_column_names <- c(deployment_raw_column_names, "detections")
```
...
```{r}
deployment %<>% mutate(startDate = format(as.POSIXct(deployments_deploy_date_time), format = "%Y-%m-%dZ"))
...
```{r}
deployment %<>% select(-one_of(deployment_raw_column_names))
```

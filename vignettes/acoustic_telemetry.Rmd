...
However, keep in mind that there is a fundamental differnce between the arguments `acoustic_tag_id` and `tag_serial_number`: the `tag_serial_number` identifies the _instrument_, which could contain multiple tags, i.e. multiple `acoustic_tag_id` values. Not only, the argument `acoustic_tag_id` is only valid when you are dealing with acoustic telemetry data (`get_acoustic_*()` functions).

## Acoutic network projects

The detections of Wels Catfish have been possible thanks to one or more acoustic network projects, mentioned in field `acoustic_project_code`:
...
To get more information about these acoustic networks, yo ucan use function `get_acoustic_projects()`
...
As for animal projects, you can retrieve the full list of acoustic network projects you have access to by using a `list_*` function:
...
More information about them can be retrieved via `get_acoustic_deployments()` function:
...
We can for example create a map of the involved stations showing the station name and the acoustic project code it belongs to as popups. First, we retrieve the station geographical coordinates:
...
n_silurus_station %>%
  ...
  addLegend(title = "Number of detected fishes",
            pal = pal,
            values = ~n)
```

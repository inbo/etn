...
In this vignette we show a typical workflow using `etn` package to retrieve acoustic telemetry data and the [`dplyr`](https://dplyr.tidyverse.org/) package (part of the [tidyverse](https://www.tidyverse.org/) R pacakges collection)  to perform basic data exploration. We will also use ome other packages: [`lubridate`](https://lubridate.tidyverse.org/) to handle date-time data and [`leaflet`](https://rstudio.github.io/leaflet/) and [`htmltools`](https://rstudio.github.io/htmltools/index.html) for interactive visualizations.
...
By using `get_animals()` you can retrieve information about each fish (`animal_id`), such as scientific name and length, capture and release date  and location and the acoustic transmitter (_tag_):
...
On a similar way we can visualie the number of detected fishes per station, `n_silurus_station`:

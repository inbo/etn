...
- `ETN_PROTOCOL` (value: `"opencpu"`, `"localdb"` or `"public"`): etn will automatically detect how to best connect to the ETN database, i.e. it will use a local database connection when available. This option allows you to override this behaviour by hardcoding the protocol to use. When no credentials are set, etn will use public (open) data only.
